...
def train_model(model, train_loader, val_loader, device, num_epochs=5):
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_accuracy:
            ...
            torch.save(model.state_dict(), "resnet50_plantnet.pth")
...
def main():
    ...
    train_transform = transforms.Compose([
        transforms.Resize((256, 256)),  # ResNet typically uses 224x224 but we start larger for cropping
        transforms.RandomCrop((224, 224)),  # Standard ResNet input size
    ...
    # Create data loaders - smaller batch size for ResNet-50 due to higher memory usage
    batch_size = 4  # Reduced from 8 due to ResNet-50's larger memory footprint
    ...
    mode = "train"  # Change to "inference" after training
    ...
    if mode == "train":
        ...
        model.load_state_dict(torch.load("resnet50_plantnet.pth"))
